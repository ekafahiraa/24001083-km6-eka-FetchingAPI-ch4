  ...
  useEffect(() => {
    const fetchUpcomingMovie = async () => {
      try {
        ...
        // Lakukan loop sampai currentPage melebihi totalPages
        while (currentPage <= totalPages) {
          const response = await axios.get(
            `https://api.themoviedb.org/3/movie/upcoming?api_key=${API_KEY}&language=en-US&page=${currentPage}`
          );

          // Update totalPages jika belum diinisialisasi
          if (totalPages === 1) {
            totalPages = response.data.total_pages;
          }

          // Gabungkan hasil permintaan sekarang dengan allMovies
          allMovies = [...allMovies, ...response.data.results];
          currentPage++;
        }

        // Set state movies dengan semua film yang telah dimuat
        setMovies(allMovies);
      } catch (error) {
