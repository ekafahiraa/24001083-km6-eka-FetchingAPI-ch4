...
const MOVIES_TO_DISPLAY = 100; // Jumlah film yang ingin ditampilkan
...
export default function NowPlayingMovie() {
  ...
  useEffect(() => {
    const fetchNowPlayingMovies = async () => {
      try {
        let allMovies = [];
        let totalPages = Math.ceil(MOVIES_TO_DISPLAY / 20); // 20 adalah jumlah film per halaman

        // Ambil data dari setiap halaman hingga mencapai jumlah film yang diinginkan
        for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
          ...
          // Jika sudah mencapai jumlah film yang diinginkan, hentikan pengambilan data
          if (allMovies.length >= MOVIES_TO_DISPLAY) break;
        }

        // Potong array movies jika melebihi jumlah film yang diinginkan
        allMovies = allMovies.slice(0, MOVIES_TO_DISPLAY);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error searching playing movies: ", error);
    }
  };
          ...
          <div className="flex flex-col gap-4 items-center">
            ...
            <div className="relative mb-5">
              <form onSubmit={handleSubmit}>
                <input
                  type="text"
                  placeholder="Search for a playing movie here..."
                  value={query}
